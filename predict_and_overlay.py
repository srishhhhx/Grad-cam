...
def main():
    parser = argparse.ArgumentParser(description='Predict and overlay segmentation mask on image')
    parser.add_argument('--image_path', type=str, help='Path to input image (if not provided, uses first test image)')
    parser.add_argument('--model_path', type=str, default='models_20250609_105424/best_model.pth', 
                       help='Path to trained model')
    parser.add_argument('--output_dir', type=str, default='prediction_overlay_results', 
                       help='Output directory for results')
    parser.add_argument('--threshold', type=float, default=0.5, help='Prediction threshold')
    parser.add_argument('--alpha', type=float, default=0.3, help='Overlay transparency (0-1)')
    parser.add_argument('--image_size', type=int, default=640, help='Input image size for model (must match training!)')
    parser.add_argument('--encoder', type=str, default='efficientnet-b5', help='Encoder architecture')
    ...
    fig.suptitle(f'Segmentation Results for {os.path.basename(args.image_path)}', fontsize=16)
    ...
    # Save the result
    output_path = os.path.join(args.output_dir, f'prediction_overlay_{os.path.basename(args.image_path)}.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Results saved to: {output_path}")
    
    # Also save individual images
    cv2.imwrite(os.path.join(args.output_dir, f'original_{os.path.basename(args.image_path)}'), 
                cv2.cvtColor(original_image, cv2.COLOR_RGB2BGR))
    cv2.imwrite(os.path.join(args.output_dir, f'overlay_{os.path.basename(args.image_path)}'), 
                cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR))
    cv2.imwrite(os.path.join(args.output_dir, f'mask_{os.path.basename(args.image_path)}'), 
                binary_mask * 255)
