...
class PDFReportGenerator:
    ...
    async def generate_report(
        ...
        story.extend(self._build_enhanced_analysis_summary(prediction_data, xai_data, include_integrated_gradients))
        ...
        if xai_data:
            story.extend(self._build_xai_section(xai_data, include_integrated_gradients))
    ...
    def _build_medical_header(self, report_id: str) -> List:
        ...
        header_table_data = [
            ...
            ["AI-Assisted Uterine Fibroids Analysis", "", f"📋 Report ID: {report_id[:8]}..."],
            ["Advanced Neural Network Diagnostics", "", f"📅 Generated: {datetime.now().strftime('%B %d, %Y at %I:%M %p')}"]
        ]

        header_table = Table(header_table_data, colWidths=[4.0*inch, 1.0*inch, 2.5*inch])
        ...
        doctor_info_table_data = [
            ["👨‍⚕️ Attending Physician:", "Dr. Shashi", "🎓 Specialization:", "Radiology & Medical Imaging"],
            ["🏥 Institution:", "Medical Imaging Center", "📅 Date of Analysis:", datetime.now().strftime('%B %d, %Y')],
            ["📧 Contact:", "imaging@medcenter.com", "🔬 AI Model:", "U-Net++ with EfficientNet-B5"]
        ]

        doctor_table = Table(doctor_info_table_data, colWidths=[1.9*inch, 2.1*inch, 1.6*inch, 1.9*inch])
        doctor_table.setStyle(TableStyle([
            ...
            ('TEXTCOLOR', (1, 0), (1, 0), self.accent_color),  # Doctor name highlighted
        ...
        content.append(HRFlowable(width="100%", thickness=3, lineCap='round', color=self.primary_color))
        content.append(Spacer(1, 3))
        content.append(HRFlowable(width="60%", thickness=1, lineCap='round', color=self.accent_color))
        content.append(Spacer(1, 30))

        return content

    def _create_segmentation_overlay(self, image_data: Dict[str, Any], prediction_data: Dict[str, Any]) -> Optional[io.BytesIO]:
        """Create ultrasound image with segmentation overlay."""
        try:
            print(f"🔍 Creating overlay - Image data keys: {list(image_data.keys())}")
            print(f"🔍 Creating overlay - Prediction data keys: {list(prediction_data.keys())}")
            ...
            if mask_b64:
                ...
                if mask_image.size != original_image.size:
                    mask_image = mask_image.resize(original_image.size, PILImage.Resampling.LANCZOS)
                ...
                for y in range(mask_gray.shape[0]):
                    for x in range(mask_gray.shape[1]):
                        ...
                        if intensity > 0.1:  # Threshold for visibility
                            ...
                            alpha = int(intensity * 180)  # Max 180 for semi-transparency
                            color = (255, int(165 * intensity), 0, alpha)  # Orange to red gradient
    ...
    def _build_scan_section(self, image_data: Dict[str, Any], prediction_data: Dict[str, Any]) -> List:
        """Build beautiful ultrasound scan section with segmentation overlay."""
        content = []

        # Enhanced section header with icon-like styling
        content.append(Paragraph("🔬 Ultrasound Scan Analysis", self.styles['SectionHeader']))
        ...
        scan_info = [
            ...
            ['Image Resolution:', f"{image_data.get('width', 'N/A')} x {image_data.get('height', 'N/A')} pixels"],
        ...
        if overlay_image:
            try:
                ...
                img_table = Table(img_frame_data, colWidths=[7.0*inch],
                                rowHeights=[0.4*inch, 0.1*inch, 4.2*inch, 0.1*inch, 0.3*inch])
        ...
        content.append(Paragraph(
            "<b>🔍 Segmentation Analysis Methodology:</b>",
    ...
    def _build_enhanced_analysis_summary(self, prediction_data: Dict[str, Any], xai_data: Optional[Dict[str, Any]], include_integrated_gradients: bool) -> List:
        """Build beautiful enhanced analysis summary section with detailed metrics."""
        content = []

        content.append(Paragraph("📊 Comprehensive Analysis Summary", self.styles['SectionHeader']))
        ...
        if prediction.get('fibroidDetected'):
            status_text = f"🔴 <font color='{self.error_color}'><b>FIBROIDS DETECTED</b></font>"
        ...
        content.append(Paragraph("🎯 <b>Primary Analysis Metrics</b>", self.styles['Highlight']))
        content.append(Spacer(1, 10))
        ...
        primary_metrics = [
            ['🤖 Model Confidence:', f"{confidence_icon} <font color='{confidence_color}'><b>{confidence:.1f}%</b></font>"],
            ['🔢 Number of Fibroids Detected:', f"<b>{fibroid_count}</b>"],
        ]
        ...
        if fibroid_count > 0:
            ...
            if fibroid_areas:
                ...
                most_severe = max(severities, key=lambda x: ['mild', 'moderate', 'severe'].index(x) if x in ['mild', 'moderate', 'severe'] else 0)

                # Enhanced metrics with icons and color coding
                severity_color = self.success_color if most_severe == 'mild' else (
                    self.warning_color if most_severe == 'moderate' else self.error_color
                )
                severity_icon = "🟢" if most_severe == 'mild' else (
                    "🟡" if most_severe == 'moderate' else "🔴"
                )

                primary_metrics.extend([
                    ['📏 Total Affected Area:', f"<b>{total_area:.1f} mm²</b>"],
                    ['📊 Average Fibroid Size:', f"<b>{avg_size:.1f} mm²</b>"],
                    ['📈 Largest Fibroid Size:', f"<b>{largest_size:.1f} mm²</b>"],
                    ['⚠️ Highest Severity Level:', f"{severity_icon} <font color='{severity_color}'><b>{most_severe.title()}</b></font>"],
                    ['📋 Severity Distribution:', f"<font color='{self.success_color}'>Mild: {severity_counts['mild']}</font>, <font color='{self.warning_color}'>Moderate: {severity_counts['moderate']}</font>, <font color='{self.error_color}'>Severe: {severity_counts['severe']}</font>"]
                ])
        ...
        if xai_data:
            content.append(Spacer(1, 15))
            content.append(Paragraph("🧠 <b>AI Explainability Metrics</b>", self.styles['Highlight']))
            content.append(Spacer(1, 10))
            ...
            if 'gradcam' in analysis:
                ...
                if gradcam_stats:
                    ...
                    focus_color = self.success_color if attention_ratio > 2.0 else (
                        self.warning_color if attention_ratio > 1.0 else self.error_color
                    )
                    focus_icon = "🎯" if attention_ratio > 2.0 else (
                        "⚡" if attention_ratio > 1.0 else "📡"
                    )

                    xai_metrics.extend([
                        ['🔍 GradCAM Attention Focus:', f"<b>{attention_mean:.4f}</b>"],
                        ['📊 GradCAM Attention Variability:', f"<b>{attention_std:.4f}</b>"],
                        ['🎯 GradCAM Concentration Ratio:', f"<font color='{focus_color}'><b>{attention_ratio:.2f}</b></font>"],
                    ])
                    ...
                    xai_metrics.append(['🧠 GradCAM Focus Interpretation:', focus_interpretation])
            ...
            if 'integratedGradients' in analysis:
                ...
                if ig_stats:
                    ...
                    attr_color = self.success_color if attr_strength > 0.001 else (
                        self.warning_color if attr_strength > 0.0001 else self.secondary_color
                    )

                    xai_metrics.extend([
                        ['🔬 IG Attribution Mean:', f"<font color='{attr_color}'><b>{attr_mean:.6f}</b></font>"],
                        ['📈 IG Attribution Std Dev:', f"<b>{attr_std:.6f}</b>"],
                        ['📊 IG Attribution Range:', f"<b>{attr_min:.6f}</b> to <b>{attr_max:.6f}</b>"],
                    ])
            elif include_integrated_gradients:
                xai_metrics.append(['🔬 Integrated Gradients:', f"<font color='{self.warning_color}'><b>Not performed (optional analysis)</b></font>"])
            ...
            if xai_metrics:
                ...
                xai_table.setStyle(TableStyle([
                    ...
                    ('BACKGROUND', (0, 0), (-1, 0), HexColor('#e0f2fe')),  # Light blue
                    ('BACKGROUND', (0, 2), (-1, 2), HexColor('#e0f2fe')),
                    ('BACKGROUND', (0, 4), (-1, 4), HexColor('#e0f2fe')),
                    ('BACKGROUND', (0, 1), (-1, 1), HexColor('#f0f9ff')),  # Very light blue
    ...
    def _build_detailed_findings(self, prediction_data: Dict[str, Any]) -> List:
        ...
        if not fibroid_areas:
            ...
            content.append(Paragraph("No fibroids were detected in the analyzed image.", self.styles['CustomBody']))
            content.append(Spacer(1, 16))
            return content
        ...
        table = Table(findings_data, colWidths=[1*inch, 1.5*inch, 1.5*inch, 2*inch])
        table.setStyle(TableStyle([
            ('FONTNAME', (0, 0), (-1, -1), 'Helvetica'),
            ('FONTSIZE', (0, 0), (-1, -1), 10),
    ...
    def _build_xai_section(self, xai_data: Dict[str, Any], include_integrated_gradients: bool = True) -> List:
        ...
        content.append(Paragraph("<b>Analysis Methods Performed:</b>", self.styles['Highlight']))
        methods_performed = []

        if has_gradcam:
            methods_performed.append("✓ GradCAM Analysis - Shows attention regions where the model focuses")

        if has_integrated_gradients:
            methods_performed.append("✓ Integrated Gradients Analysis - Provides pixel-level attribution analysis")
        else:
            if include_integrated_gradients:
                methods_performed.append("• Integrated Gradients Analysis - Not performed (optional)")
            else:
                methods_performed.append("• Integrated Gradients Analysis - Not performed for this report")
        ...
        if has_gradcam:
            ...
            if gradcam_stats:
                content.append(Paragraph("<b>GradCAM Analysis Results:</b>", self.styles['Highlight']))

                attention_mean = gradcam_stats.get('attentionMean', 0)
                attention_ratio = gradcam_stats.get('attentionRatio', 0)

                content.append(Paragraph(f"• Attention Focus Score: {attention_mean:.3f}", self.styles['CustomBody']))
                content.append(Paragraph(f"• Attention Concentration Ratio: {attention_ratio:.2f}", self.styles['CustomBody']))

                if attention_ratio > 2.0:
                    content.append(Paragraph("• Model shows good focus on relevant regions", self.styles['CustomBody']))
                elif attention_ratio < 1.0:
                    content.append(Paragraph("• Model attention is distributed across the image", self.styles['CustomBody']))
                else:
                    content.append(Paragraph("• Model shows moderate focus on specific regions", self.styles['CustomBody']))
        ...
        if explanation.get('confidence'):
            ...
            content.append(Paragraph(f"<b>Explanation Confidence:</b> {confidence:.1f}%", self.styles['Highlight']))
    ...
    def _build_recommendations(self, prediction_data: Dict[str, Any], xai_data: Optional[Dict[str, Any]]) -> List:
        ...
        if prediction.get('fibroidDetected'):
            ...
            if any(area.get('severity') == 'severe' for area in fibroid_areas):
                recommendations.insert(1, "🚨 Large fibroids detected - consider urgent gynecological consultation.")
        else:
            recommendations = [
        ...
        if xai_data:
            xai_recommendations = xai_data.get('analysis', {}).get('explanation', {}).get('recommendations', [])
    ...
    def _build_footer(self) -> List:
        ...
        signature_table_data = [
            ["🔬 Advanced AI Medical Imaging Analysis System", f"📋 Document ID: {datetime.now().strftime('%Y%m%d_%H%M%S')}"]
        ]

        signature_table = Table(signature_table_data, colWidths=[4.5*inch, 3.0*inch])
